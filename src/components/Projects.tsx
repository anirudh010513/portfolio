  ...
  return (
    <div className="flex flex-wrap gap-2">
      {stack.map((tech) => (
        <span
          key={tech}
          className="rounded-lg border border-zinc-200 bg-zinc-50 px-2.5 py-1 font-mono text-[11px] font-medium text-zinc-600 dark:border-white/10 dark:bg-white/5 dark:text-zinc-400"
        >
          {tech}
        </span>
      ))}
...
function FeaturedCard({ project }: { project: Project }) {
  return (
    <Reveal>
      <div className="rounded-[26px] bg-gradient-to-br from-indigo-500/50 via-zinc-200/60 to-cyan-400/50 p-px dark:via-white/10 dark:from-indigo-400/40 dark:to-cyan-400/40">
        <Spotlight>
          <motion.article
            whileHover={{ y: -4 }}
            transition={{ duration: 0.25, ease: 'easeOut' }}
            className="group relative overflow-hidden rounded-[25px] bg-white p-6 dark:bg-[#0d0d15] sm:p-9"
          >
            <div
              aria-hidden="true"
            ...
            <div className="relative">
              <div className="flex flex-wrap items-center gap-3">
                ...
                {project.year ? (
                  <span className="rounded-full border border-zinc-200 px-3.5 py-1.5 font-mono text-xs font-medium text-zinc-500 dark:border-white/10 dark:text-zinc-400">
                    {project.year}
                  </span>
                ) : null}
              </div>

              <h3 className="mt-6 font-display text-2xl font-semibold tracking-tight text-zinc-900 dark:text-white sm:text-3xl">
                {project.name}
              </h3>
              <p className="mt-2 text-sm font-semibold uppercase tracking-wider text-indigo-600 dark:text-indigo-300">
                {project.tagline}
              </p>
      ...
      <Spotlight className="h-full">
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <span className="h-px w-10 bg-gradient-to-r from-indigo-500/60 to-transparent" aria-hidden="true" />
          </div>
          <h3 className="mt-4 font-display text-lg font-semibold tracking-tight text-zinc-900 transition-colors group-hover:text-indigo-600 dark:text-white dark:group-hover:text-indigo-300">
            {project.name}
          </h3>
          <p className="mt-1 text-xs font-semibold uppercase tracking-wider text-zinc-400 dark:text-zinc-500">
            {project.tagline}
          </p>
